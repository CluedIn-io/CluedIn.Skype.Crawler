...
{
    public class GetCrawlJobDataBehaviour : SkypeProviderTest
    {
        ...
        [Theory]
        [InlineAutoData]
        public async Task SkypeCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Skype.Core.SkypeCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
